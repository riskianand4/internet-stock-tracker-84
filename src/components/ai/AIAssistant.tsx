...
const AIAssistant: React.FC<AIAssistantProps> = ({
  ...
  const getAIResponse = async (userMessage: string): Promise<{
    ...
    try {
      ...
      if (isConfigured && isOnline && apiService) {
        ...
        if (response.success) {
          return {
            text: (response.data as any)?.response || 'Sorry, I cannot process your request right now.',
  ...
  if (!isOpen) {
    return <motion.div className={`fixed ${isMobile ? 'bottom-4 right-4' : 'bottom-6 right-6'} z-50 ${className}`} initial={{
      scale: 0
    }} animate={{
      scale: 1
    }} transition={{
      ...
      damping: 20
        ...
        <Button onClick={() => setIsOpen(true)} size="lg" className={`rounded-full ${isMobile ? 'h-12 w-12' : 'h-14 w-14'} shadow-lg hover:shadow-xl transition-all duration-300 bg-gradient-to-r from-primary to-primary/80 my-[73px]`}>
  ...
  return <motion.div className={`fixed ${isMobile ? 'bottom-4 right-4' : 'bottom-6 right-6'} z-50 ${className}`} initial={{
    ...
    damping: 20
      ...
      <Card className={`${isMobile ? 'w-[calc(100vw-2rem)] max-w-sm' : 'w-96'} shadow-xl border-2 ${isMinimized ? 'h-16' : isMobile ? 'h-[400px]' : 'h-[500px]'} transition-all duration-300`}>
        <CardHeader className={`${isMobile ? 'p-3' : 'p-4'} border-b bg-gradient-to-r from-primary/10 to-primary/5`}>
          <div className="flex items-center justify-between">
            <div className={`flex items-center ${isMobile ? 'gap-1' : 'gap-2'}`}>
              <div className={`${isMobile ? 'w-1.5 h-1.5' : 'w-2 h-2'} bg-green-500 rounded-full animate-pulse`}></div>
              <CardTitle className={`${isMobile ? 'text-sm' : 'text-md'}`}>AI Assistant</CardTitle>
            ...
            <div className={`flex items-center ${isMobile ? 'gap-0.5' : 'gap-1'}`}>
              <Button variant="ghost" size="sm" onClick={() => setIsMinimized(!isMinimized)}>
                {isMinimized ? <Maximize2 className={`${isMobile ? 'h-3 w-3' : 'h-4 w-4'}`} /> : <Minimize2 className={`${isMobile ? 'h-3 w-3' : 'h-4 w-4'}`} />}
              </Button>
        ...
        <AnimatePresence>
          ...
          duration: 0.3
              ...
              <CardContent className={`p-0 flex flex-col ${isMobile ? 'h-[336px]' : 'h-[436px]'}`}>
                <ScrollArea className={`flex-1 ${isMobile ? 'p-3' : 'p-4'}`}>
                  ...
                  y: 0
                        ...
                        <div className={`max-w-[80%] ${isMobile ? 'p-2' : 'p-3'} rounded-lg ${message.isUser ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                        ...
                        minute: '2-digit'
                          ...
                          {message.suggestions && <div className={`${isMobile ? 'mt-1 space-y-0.5' : 'mt-2 space-y-1'}`}>
                              {message.suggestions.map((suggestion, index) => <Button key={index} variant="outline" size="sm" className={`text-xs ${isMobile ? 'h-5' : 'h-6'} mr-1 mb-1`} onClick={() => handleSuggestionClick(suggestion)}>
                                  {suggestion}
                                </Button>)}
                            </div>}
                        </div>
                  ...
                  y: 0
                        ...
                        <div className={`bg-muted ${isMobile ? 'p-2' : 'p-3'} rounded-lg`}>
                          <div className={`flex ${isMobile ? 'space-x-0.5' : 'space-x-1'}`}>
                            <div className={`${isMobile ? 'w-1.5 h-1.5' : 'w-2 h-2'} bg-muted-foreground rounded-full animate-bounce`}></div>
                            <div className={`${isMobile ? 'w-1.5 h-1.5' : 'w-2 h-2'} bg-muted-foreground rounded-full animate-bounce`} style={{
                        animationDelay: '0.1s'
                      }}></div>
                            <div className={`${isMobile ? 'w-1.5 h-1.5' : 'w-2 h-2'} bg-muted-foreground rounded-full animate-bounce`} style={{
                        animationDelay: '0.2s'
                ...
                <div className={`${isMobile ? 'p-3' : 'p-4'} border-t`}>
                  <div className={`flex ${isMobile ? 'gap-1' : 'gap-2'}`}>
                    <Input ref={inputRef} placeholder={isMobile ? "Tanyakan..." : "Tanyakan tentang stok, laporan, atau fitur..."} value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyPress={handleKeyPress} className={`flex-1 ${isMobile ? 'text-xs' : ''}`} />
                    <Button variant="outline" size="sm" onClick={toggleVoiceInput} className={`${isListening ? 'bg-red-100 hover:bg-red-200' : ''} ${isMobile ? 'px-2' : ''}`}>
                      {isListening ? <MicOff className={`${isMobile ? 'h-3 w-3' : 'h-4 w-4'}`} /> : <Mic className={`${isMobile ? 'h-3 w-3' : 'h-4 w-4'}`} />}
                    </Button>
                    <Button onClick={handleSendMessage} size="sm" disabled={!inputValue.trim() || isTyping} className={isMobile ? 'px-2' : ''}>
